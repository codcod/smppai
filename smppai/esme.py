...
class Esme:
    ...
    def send_submit_sm(self, src: str, dest: str, message: str):
        ...
        for cmd in cmds:
            ...
            resp = self.recv_pdu()
            match type(resp):
                case smpplib.command.SubmitSMResp:
                    logger.debug('<submit_sm_resp> received')
                case smpplib.command.DeliverSM:
                    logger.debug('<deliver_sm> received')
                case _:
