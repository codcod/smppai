...
| `0xF0`–`0xF7` | GSM message class | GSM 03.38 when bit `0x04` is clear, `utf-8` when set |

`short_message` is capped at 254 octets on every coding (SMPP v3.4 §5.2.21), and every
value in the table, `0xF0`–`0xF7` included, is accepted on encode. Single-part limits
...
`0xC0`–`0xEF`, `0xF8`–`0xFF` (bit 3 is reserved in GSM 03.38) and other reserved values are
rejected outright. Text that the chosen coding
